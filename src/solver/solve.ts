...
function delegationGuide(task:PipelineTask,input:ProjectInput):AgentDelegationGuide{
 ...
 const canonicalVisualMatches=visualProfile?.ontologyMatches?.map((match)=>`${match.label??match.conceptId} (${match.conceptId})`).join(', ');
 const visualContext=visualProfile?tr(input.language,
  `User-approved visual profile — ${visualProfile.summary}. Principles: ${visualProfile.designPrinciples.map((item)=>`${item.principle}: ${item.application}`).join('; ')}. Motion cues: ${visualProfile.motion.suggestedBehaviors.join('; ')||'none stated'}.${canonicalVisualMatches?` Canonical quanda.skills matches: ${canonicalVisualMatches}.`:''} Treat this as direction, not permission to copy a reference.`,
  `Hồ sơ hình ảnh đã được người dùng duyệt — ${visualProfile.summary}. Nguyên tắc: ${visualProfile.designPrinciples.map((item)=>`${item.principle}: ${item.application}`).join('; ')}. Gợi ý chuyển động: ${visualProfile.motion.suggestedBehaviors.join('; ')||'không nêu'}.${canonicalVisualMatches?` Khái niệm chuẩn từ quanda.skills: ${canonicalVisualMatches}.`:''} Xem đây là định hướng, không phải quyền sao chép hình tham khảo.`):null;
